...
//! Demonstrates how zquic can use TokioZ for async I/O, timers, and task management
...
pub const QuicConnection = struct {
    ...
    waker: ?*tokioz.scheduler.Waker = null,
    ...
    pub fn startHandshake(self: *Self) !void {
        ...
        const runtime = tokioz.Runtime.global() orelse return error.NoRuntime;
...
/// Example: QUIC echo server using TokioZ
pub fn quicEchoServer(port: u16) !void {
    ...
    while (true) {
        ...
        _ = try tokioz.spawnUrgent(handleQuicConnection, .{conn});
    }
}
...
/// Example: QUIC client using TokioZ
pub fn quicClient(server_addr: std.net.Address) !void {
    ...
    try tokioz.sleep(200);

    // Send test messages
    const messages = [_][]const u8{ "Hello", "QUIC", "World!", "via", "TokioZ" };
    ...
    for (messages) |msg| {
        ...
        try tokioz.sleep(100); // Small delay between messages
    }
}

/// Comprehensive QUIC + TokioZ integration test
pub fn quicIntegrationTest() !void {
    std.debug.print("\n🔥 QUIC + TokioZ Integration Test\n", .{});
    std.debug.print("==================================\n", .{});

    // Start QUIC server in background
    _ = try tokioz.spawnUrgent(quicEchoServer, .{@as(u16, 9001)});
    ...
    _ = try tokioz.spawn(quicClient, .{server_addr});

    // Let them run for a bit
    try tokioz.sleep(2000);
